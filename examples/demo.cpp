...
std::string ToHex(const std::vector<CK_BYTE>& bytes,
...
int main() {
  // Swap this block for `SessionManager sm("/path/to/vendor-pkcs11.so", slotId,
  // pin);` to run the identical demo against a real token.
  mock::Reset();
  ...
  if (auto best = mechanisms.SelectBestMechanism(0, "encrypt", CKK_AES, 256)) {
    std::cout << "Best AES-256 encrypt mechanism: 0x" << std::hex << *best
              << std::dec << "\n";
  }
  ...
  aes_params.can_encrypt_ = aes_params.can_decrypt_ = aes_params.can_wrap_ =
      aes_params.can_unwrap_ = true;
  CK_OBJECT_HANDLE aes_key =
      key_manager.GenerateSecretKey(session, functions, aes_params);
  ...
  for (std::size_t i = 0; i < read_back.Size(); ++i) {
    std::cout << "  "
              << AttributeManager::DescribeAttribute(read_back.Data()[i].type)
              << "\n";
  }
  ...
  std::vector<CK_BYTE> message = {'h', 'e', 'l', 'l', 'o', ' ',
                                  'p', 'k', 'c', 's', '1', '1'};
  auto sign_ids = processor.SubmitSigningBatch({message}, rsa_pair.private_key_,
                                               CKM_SHA256_RSA_PKCS);
  ...
  processor.Stop();
  std::cout << "Submitted " << sign_ids.size()
            << " signing operation(s): " << sign_ids.front() << "\n";
  ...
  CK_ULONG enc_len = 0;
  functions->C_Encrypt(session, message.data(),
                       static_cast<CK_ULONG>(message.size()), nullptr,
                       &enc_len);
  std::vector<CK_BYTE> ciphertext(enc_len);
  functions->C_Encrypt(session, message.data(),
                       static_cast<CK_ULONG>(message.size()), ciphertext.data(),
                       &enc_len);
  ...
  functions->C_Decrypt(session, ciphertext.data(), enc_len, plaintext.data(),
                       &dec_len);
  ...
  CK_OBJECT_HANDLE wrapping_key =
      key_manager.GenerateSecretKey(session, functions, wrapping_params);

  auto wrapped = transport.WrapKey(session, functions, aes_key, wrapping_key,
                                   KeyTransport::WrapMechanism::kAesKeyWrap);
  std::cout << "Wrapped key (" << wrapped.wrapped_key_.size()
            << " bytes): " << ToHex(wrapped.wrapped_key_) << "\n";
  CK_OBJECT_HANDLE unwrapped = transport.UnwrapKey(
      session, functions, wrapped, wrapping_key, "demo-aes-key-restored");
  ...
  CK_OBJECT_HANDLE derived_key =
      derivation.DeriveSp800108Key(session, functions, kdf_params);
  std::cout << "Derived key handle=" << derived_key << "\n";

  PrintHeader("Object search");
  ObjectFinder finder;
  auto matches = finder.FindObjects(
      session, functions,
      ObjectFinder::SearchCriteria().WithLabel("demo-aes-key"));
  std::cout << "Found " << matches.size()
            << " object(s) labeled \"demo-aes-key\"\n";

  PrintHeader("Health monitor");
  HealthMonitor health;
  bool passed =
      health.PerformComprehensiveTest(functions, /*slotId=*/0, "1234");
