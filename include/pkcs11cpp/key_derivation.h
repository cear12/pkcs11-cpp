...
class KeyDerivation {
 public:
  ...
  CK_OBJECT_HANDLE DeriveEcdhKey(CK_SESSION_HANDLE session,
                                 CK_FUNCTION_LIST_PTR functions,
                                 const DerivationParams& params) const;
  CK_OBJECT_HANDLE DeriveSp800108Key(CK_SESSION_HANDLE session,
                                     CK_FUNCTION_LIST_PTR functions,
                                     const DerivationParams& params) const;
  CK_OBJECT_HANDLE DerivePbkdf2Key(CK_SESSION_HANDLE session,
                                   CK_FUNCTION_LIST_PTR functions,
                                   const DerivationParams& params) const;
 ...
 private:
  static std::vector<CK_ATTRIBUTE> BuildDerivedKeyTemplate(
