...
class KeyManager {
 public:
  ...
  struct KeyGenerationParams {
    ...
    std::optional<std::vector<CK_BYTE>>
        ec_params_;  // EC only, DER-encoded OID; auto-filled if absent
  };
  ...
  KeyPair GenerateKeyPair(CK_SESSION_HANDLE session,
                          CK_FUNCTION_LIST_PTR functions,
                          const KeyGenerationParams& params) const;

  CK_OBJECT_HANDLE GenerateSecretKey(CK_SESSION_HANDLE session,
                                     CK_FUNCTION_LIST_PTR functions,
                                     const KeyGenerationParams& params) const;
 ...
 private:
  static void AddCommonKeyAttributes(
      const KeyGenerationParams& params,
      AttributeManager::AttributeSet& public_set,
      AttributeManager::AttributeSet& private_set);

  KeyPair GenerateRsaKeyPair(CK_SESSION_HANDLE session,
                             CK_FUNCTION_LIST_PTR functions,
                             const KeyGenerationParams& params) const;
  KeyPair GenerateEcKeyPair(CK_SESSION_HANDLE session,
                            CK_FUNCTION_LIST_PTR functions,
                            const KeyGenerationParams& params) const;
  CK_OBJECT_HANDLE GenerateAesKey(CK_SESSION_HANDLE session,
                                  CK_FUNCTION_LIST_PTR functions,
                                  const KeyGenerationParams& params) const;
  CK_OBJECT_HANDLE GenerateDes3Key(CK_SESSION_HANDLE session,
                                   CK_FUNCTION_LIST_PTR functions,
                                   const KeyGenerationParams& params) const;
};
