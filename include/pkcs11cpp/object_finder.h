 ...
 public:
  struct SearchCriteria {
    ...
    SearchCriteria& ForEncryption(bool required = true) {
      can_encrypt_ = required;
      return *this;
    }
  };

  explicit ObjectFinder(
      std::chrono::seconds cache_timeout = std::chrono::seconds(300))
      : cache_timeout_(cache_timeout) {}

  std::vector<CK_OBJECT_HANDLE> FindObjects(CK_SESSION_HANDLE session,
