...
AttributeManager::AttributeSet& AttributeManager::AttributeSet::AddAttribute(
  ...
  if (it != attribute_index_.end()) {
    ...
    attributes_[index].ulValueLen =
        static_cast<CK_ULONG>(attribute_data_[index].size());
  } else {
    size_t index = attributes_.size();
    attribute_index_[type] = index;
    attribute_data_.push_back(value);
    attributes_.push_back(
        {type, attribute_data_.back().data(),
         static_cast<CK_ULONG>(attribute_data_.back().size())});
  }
  return *this;
}

AttributeManager::AttributeSet& AttributeManager::AttributeSet::AddBoolean(
    CK_ATTRIBUTE_TYPE type, bool value) {
  CK_BBOOL bool_value = value ? CK_TRUE : CK_FALSE;
  return AddAttribute(
      type, std::vector<CK_BYTE>(
                reinterpret_cast<CK_BYTE*>(&bool_value),
                reinterpret_cast<CK_BYTE*>(&bool_value) + sizeof(bool_value)));
}

AttributeManager::AttributeSet& AttributeManager::AttributeSet::AddULong(
    CK_ATTRIBUTE_TYPE type, CK_ULONG value) {
  return AddAttribute(
      type,
      std::vector<CK_BYTE>(reinterpret_cast<CK_BYTE*>(&value),
                           reinterpret_cast<CK_BYTE*>(&value) + sizeof(value)));
}

AttributeManager::AttributeSet& AttributeManager::AttributeSet::AddString(
    CK_ATTRIBUTE_TYPE type, const std::string& value) {
  return AddAttribute(type, std::vector<CK_BYTE>(value.begin(), value.end()));
}

AttributeManager::AttributeSet& AttributeManager::AttributeSet::AddBytes(
    CK_ATTRIBUTE_TYPE type, const std::vector<CK_BYTE>& value) {
  return AddAttribute(type, value);
}

AttributeManager::AttributeSet& AttributeManager::AttributeSet::AddMetadata(
    const std::string& key, const std::string& value) {
  CK_ATTRIBUTE_TYPE vendor_type =
      CKA_VENDOR_DEFINED +
      static_cast<CK_ATTRIBUTE_TYPE>(std::hash<std::string>{}(key) % 1000);
  return AddString(vendor_type, key + "=" + value);
}

bool AttributeManager::AttributeSet::HasAttribute(
    CK_ATTRIBUTE_TYPE type) const {
  return attribute_index_.count(type) > 0;
}

std::optional<std::vector<CK_BYTE>>
AttributeManager::AttributeSet::GetAttribute(CK_ATTRIBUTE_TYPE type) const {
  auto it = attribute_index_.find(type);
  if (it != attribute_index_.end()) {
...
const std::vector<CK_ATTRIBUTE_TYPE>& CommonAttributeTypes() {
  static const std::vector<CK_ATTRIBUTE_TYPE> kTypes = {
      CKA_CLASS,       CKA_KEY_TYPE, CKA_TOKEN,  CKA_PRIVATE, CKA_SENSITIVE,
      CKA_EXTRACTABLE, CKA_SIGN,     CKA_VERIFY, CKA_ENCRYPT, CKA_DECRYPT,
      CKA_WRAP,        CKA_UNWRAP,   CKA_LABEL,  CKA_ID,
  };
  return kTypes;
}
}  // namespace

AttributeManager::AttributeSet AttributeManager::ReadObjectAttributes(
  ...
  functions->C_GetAttributeValue(session, object, probe.data(),
  ...
  CK_RV rv = functions->C_GetAttributeValue(
      session, object, probe.data(), static_cast<CK_ULONG>(probe.size()));
  if (rv != CKR_OK) {
    log::Warn("C_GetAttributeValue returned rv=" + std::to_string(rv) +
              " while reading object " + std::to_string(object) +
              "; returning the attributes that were readable");
  }
...
void AttributeManager::ModifyObjectAttributes(
    CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
  ...
  for (size_t i = 0; i < new_attributes.Size(); ++i) {
    ...
    if (IsAttributeModifiable(type)) {
      ...
      log::Warn("Attempt to modify read-only attribute: " +
                DescribeAttribute(type));
    }
  }

  if (!modifiable.empty()) {
    CK_RV rv = functions->C_SetAttributeValue(
        session, object, modifiable.data(),
...
AttributeManager::AttributeSet AttributeManager::CreateRsaPrivateKeyTemplate(
    const std::string& label, const std::vector<CK_BYTE>& id, bool extractable,
    bool sensitive) {
  AttributeSet attrs;
...
AttributeManager::AttributeSet AttributeManager::CreateAesKeyTemplate(
...
std::string AttributeManager::DescribeAttribute(CK_ATTRIBUTE_TYPE type) {
  static const std::unordered_map<CK_ATTRIBUTE_TYPE, std::string> kNames = {
      {CKA_CLASS, "CKA_CLASS"},         {CKA_TOKEN, "CKA_TOKEN"},
      {CKA_PRIVATE, "CKA_PRIVATE"},     {CKA_LABEL, "CKA_LABEL"},
      {CKA_VALUE_LEN, "CKA_VALUE_LEN"}, {CKA_EXTRACTABLE, "CKA_EXTRACTABLE"},
      {CKA_SENSITIVE, "CKA_SENSITIVE"}, {CKA_ID, "CKA_ID"},
      {CKA_KEY_TYPE, "CKA_KEY_TYPE"},   {CKA_DERIVE, "CKA_DERIVE"},
      {CKA_ENCRYPT, "CKA_ENCRYPT"},     {CKA_DECRYPT, "CKA_DECRYPT"},
      {CKA_WRAP, "CKA_WRAP"},           {CKA_UNWRAP, "CKA_UNWRAP"},
      {CKA_SIGN, "CKA_SIGN"},           {CKA_VERIFY, "CKA_VERIFY"},
  };
  auto it = kNames.find(type);
  if (it != kNames.end()) return it->second;
  if (type >= CKA_VENDOR_DEFINED)
    return "CKA_VENDOR_DEFINED+" + std::to_string(type - CKA_VENDOR_DEFINED);
...
void AttributeManager::ValidateAttribute(CK_ATTRIBUTE_TYPE type,
