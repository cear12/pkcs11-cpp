...
CryptoProcessor::CryptoProcessor(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
    ...
    : session_(session), functions_(functions), max_threads_(thread_count == 0 ? 1 : thread_count) {}
...
std::vector<std::string> CryptoProcessor::SubmitSigningBatch(const std::vector<std::vector<CK_BYTE>>& data_to_sign,
...
std::vector<std::string> CryptoProcessor::SubmitEncryptionBatch(const std::vector<std::vector<CK_BYTE>>& data_to_encrypt,
    ...
    for (const auto& data : data_to_encrypt) {
        ...
        if (!iv.empty()) {
            op->signature_ = iv;  // reuse the otherwise-unused `signature` slot to carry the IV bytes
            op->mechanism_ = {mechanism_type, op->signature_.data(), static_cast<CK_ULONG>(op->signature_.size())};
        } else {
            op->mechanism_ = {mechanism_type, nullptr, 0};
        }
        op->input_data_ = data;
...
void CryptoProcessor::WorkerLoop() {
    while (running_.load()) {
        std::unique_ptr<Operation> operation;
        {
            ...
            queue_condition_.wait(lock, [this] { return !queue_.empty() || !running_.load(); });
...
void CryptoProcessor::ProcessOperation(Operation& operation) {
    try {
        switch (operation.type_) {
            case Operation::Type::kSign: ProcessSigning(operation); break;
            case Operation::Type::kVerify: ProcessVerification(operation); break;
            case Operation::Type::kEncrypt: ProcessEncryption(operation); break;
            case Operation::Type::kDecrypt: ProcessDecryption(operation); break;
            case Operation::Type::kDigest: ProcessDigest(operation); break;
        }
...
void CryptoProcessor::ProcessSigning(Operation& operation) {
    CK_RV rv = functions_->C_SignInit(session_, &operation.mechanism_, operation.key_handle_);
    if (rv != CKR_OK) { operation.result_ = rv; return; }

    CK_ULONG signature_len = 0;
    rv = functions_->C_Sign(session_, operation.input_data_.data(), static_cast<CK_ULONG>(operation.input_data_.size()),
...
void CryptoProcessor::ProcessVerification(Operation& operation) {
    CK_RV rv = functions_->C_VerifyInit(session_, &operation.mechanism_, operation.key_handle_);
    if (rv != CKR_OK) { operation.result_ = rv; return; }

    rv = functions_->C_Verify(session_, operation.input_data_.data(), static_cast<CK_ULONG>(operation.input_data_.size()),
                               operation.signature_.data(), static_cast<CK_ULONG>(operation.signature_.size()));
    operation.result_ = rv;
}

void CryptoProcessor::ProcessEncryption(Operation& operation) {
    CK_RV rv = functions_->C_EncryptInit(session_, &operation.mechanism_, operation.key_handle_);
    if (rv != CKR_OK) { operation.result_ = rv; return; }

    CK_ULONG encrypted_len = 0;
    rv = functions_->C_Encrypt(session_, operation.input_data_.data(), static_cast<CK_ULONG>(operation.input_data_.size()),
...
void CryptoProcessor::ProcessDecryption(Operation& operation) {
    CK_RV rv = functions_->C_DecryptInit(session_, &operation.mechanism_, operation.key_handle_);
    if (rv != CKR_OK) { operation.result_ = rv; return; }

    CK_ULONG decrypted_len = 0;
    rv = functions_->C_Decrypt(session_, operation.input_data_.data(), static_cast<CK_ULONG>(operation.input_data_.size()),
...
void CryptoProcessor::ProcessDigest(Operation& operation) {
    ...
    rv = functions_->C_Digest(session_, operation.input_data_.data(), static_cast<CK_ULONG>(operation.input_data_.size()),
                               nullptr, &digest_len);
    if (rv != CKR_OK) { operation.result_ = rv; return; }
