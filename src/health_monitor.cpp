...
void HealthMonitor::StartMonitoring(CK_FUNCTION_LIST_PTR functions, std::chrono::seconds interval) {
    ...
    monitor_thread_ = std::thread([this, functions, interval] {
        while (monitoring_.load()) {
            ...
            shutdown_signal_.wait_for(lock, interval, [this] { return !monitoring_.load(); });
        }
    });
}
...
HealthMonitor::HealthReport HealthMonitor::GetHealthReport() const {
    ...
    for (const auto& [slotId, slotInfo] : slots_) {
        ...
        for (const auto& issue : slotInfo.issues_) {
            auto& bucket = (issue.rfind("CRITICAL", 0) == 0) ? report.critical_issues_ : report.warnings_;
...
void HealthMonitor::CheckSlotHealth(CK_FUNCTION_LIST_PTR functions, CK_SLOT_ID slot_id) {
    std::lock_guard<std::mutex> lock(monitor_mutex_);
    ...
    if (rv != CKR_OK) {
        slot.issues_.push_back("CRITICAL: cannot get slot info (rv=" + std::to_string(rv) + ")");
        slot.healthy_ = false;
        return;
    }
    ...
    if (rv != CKR_OK) {
        slot.issues_.push_back("CRITICAL: cannot get token info (rv=" + std::to_string(rv) + ")");
        slot.healthy_ = false;
        return;
    }
    ...
    if (slot.token_info_.ulFreePrivateMemory != CK_UNAVAILABLE_INFORMATION &&
        ...
        slot.issues_.push_back("WARNING: low free private memory (" +
    ...
    if (slot.token_info_.ulFreePublicMemory != CK_UNAVAILABLE_INFORMATION &&
        ...
        slot.issues_.push_back("WARNING: low free public memory (" +
                               std::to_string(slot.token_info_.ulFreePublicMemory) + " bytes)");
    }
}

bool HealthMonitor::PerformComprehensiveTest(CK_FUNCTION_LIST_PTR functions, CK_SLOT_ID slot_id,
                                              const std::string& user_pin) {
    try {
        auto start_time = std::chrono::high_resolution_clock::now();

        CK_SESSION_HANDLE session;
        CK_RV rv = functions->C_OpenSession(slot_id, CKF_SERIAL_SESSION | CKF_RW_SESSION, nullptr, nullptr, &session);
        if (rv != CKR_OK) {
        ...
        CK_ATTRIBUTE key_template[] = {
            ...
            {CKA_VALUE_LEN, &key_length, sizeof(key_length)}, {CKA_TOKEN, &false_value, sizeof(false_value)},
            {CKA_ENCRYPT, &true_value, sizeof(true_value)},   {CKA_DECRYPT, &true_value, sizeof(true_value)},
        };
        ...
        rv = functions->C_GenerateKey(session, &key_gen_mech, key_template,
                                       sizeof(key_template) / sizeof(CK_ATTRIBUTE), &test_key);
        if (rv != CKR_OK) {
                ...
                if (rv == CKR_OK) {
                    ...
                    if (rv == CKR_OK) {
                        ...
                        if (rv == CKR_OK) {
                            ...
                            rv = functions->C_Decrypt(session, ciphertext.data(), ciphertext_len, decrypted.data(),
                                                       &decrypted_len);
