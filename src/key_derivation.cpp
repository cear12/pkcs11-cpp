...
std::vector<CK_ATTRIBUTE> KeyDerivation::BuildDerivedKeyTemplate(const DerivationParams& params,
    ...
    if (!params.derived_key_label_.empty()) {
        storage.emplace_back(params.derived_key_label_.begin(), params.derived_key_label_.end());
...
CK_OBJECT_HANDLE KeyDerivation::DeriveEcdhKey(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
    ...
    ecdh_params.ulPublicDataLen = static_cast<CK_ULONG>(params.peer_public_key_.size());
    ecdh_params.pPublicData = const_cast<CK_BYTE*>(params.peer_public_key_.data());

    CK_MECHANISM mechanism = {CKM_ECDH1_DERIVE, &ecdh_params, sizeof(ecdh_params)};

    std::vector<std::vector<CK_BYTE>> storage;
    auto tmpl = BuildDerivedKeyTemplate(params, storage);

    CK_OBJECT_HANDLE derived_key;
    CK_RV rv = functions->C_DeriveKey(session, &mechanism, params.base_key_, tmpl.data(),
...
CK_OBJECT_HANDLE KeyDerivation::DeriveSp800108Key(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
                                                    const DerivationParams& params) const {
    ...
    fixed_data.insert(fixed_data.end(), params.label_.begin(), params.label_.end());
    fixed_data.push_back(0x00);
    fixed_data.insert(fixed_data.end(), params.context_.begin(), params.context_.end());
    ...
    CK_MECHANISM mechanism = {CKM_SP800_108_COUNTER_KDF, &kdf_params, sizeof(kdf_params)};
...
CK_OBJECT_HANDLE KeyDerivation::DerivePbkdf2Key(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
    ...
    pbkdf2_params.ulSaltSourceDataLen = static_cast<CK_ULONG>(params.salt_.size());
    ...
    pbkdf2_params.pPassword = reinterpret_cast<CK_UTF8CHAR_PTR>(const_cast<char*>(params.password_.c_str()));
    pbkdf2_params.ulPasswordLen = static_cast<CK_ULONG>(params.password_.length());

    CK_MECHANISM mechanism = {CKM_PKCS5_PBKD2, &pbkdf2_params, sizeof(pbkdf2_params)};
