...
CK_ULONG RsaModulusBits(KeyManager::KeyAlgorithm algorithm) {
    switch (algorithm) {
        case KeyManager::KeyAlgorithm::kRsa2048: return 2048;
        case KeyManager::KeyAlgorithm::kRsa3072: return 3072;
        case KeyManager::KeyAlgorithm::kRsa4096: return 4096;
...
CK_ULONG AesKeyBits(KeyManager::KeyAlgorithm algorithm) {
    switch (algorithm) {
        case KeyManager::KeyAlgorithm::kAes128: return 128;
        case KeyManager::KeyAlgorithm::kAes192: return 192;
        case KeyManager::KeyAlgorithm::kAes256: return 256;
        default: throw std::invalid_argument("aesKeyBits: not an AES algorithm");
    }
}
}  // namespace

void KeyManager::AddCommonKeyAttributes(const KeyGenerationParams& params, AttributeManager::AttributeSet& public_set,
...
KeyManager::KeyPair KeyManager::GenerateRsaKeyPair(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
    ...
    std::vector<CK_BYTE> public_exponent = params.public_exponent_.value_or(std::vector<CK_BYTE>{0x01, 0x00, 0x01});
    ...
    private_set.AddULong(CKA_CLASS, CKO_PRIVATE_KEY).AddULong(CKA_KEY_TYPE, CKK_RSA);
    ...
    CK_RV rv = functions->C_GenerateKeyPair(session, &mechanism, public_set.Data(),
...
KeyManager::KeyPair KeyManager::GenerateEcKeyPair(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
                                                   const KeyGenerationParams& params) const {
    AttributeManager::AttributeSet public_set, private_set;
    std::vector<CK_BYTE> ec_params = params.ec_params_.value_or(DefaultEcParams(params.algorithm_));

    public_set.AddULong(CKA_CLASS, CKO_PUBLIC_KEY).AddULong(CKA_KEY_TYPE, CKK_ECDSA).AddBytes(CKA_EC_PARAMS, ec_params);
    if (params.can_verify_) public_set.AddBoolean(CKA_VERIFY, true);

    private_set.AddULong(CKA_CLASS, CKO_PRIVATE_KEY).AddULong(CKA_KEY_TYPE, CKK_ECDSA);
    if (params.can_sign_) private_set.AddBoolean(CKA_SIGN, true);
...
CK_OBJECT_HANDLE KeyManager::GenerateAesKey(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
    ...
    CK_RV rv =
        functions->C_GenerateKey(session, &mechanism, key_set.Data(), static_cast<CK_ULONG>(key_set.Size()), &key);
    if (rv != CKR_OK) {
...
bool KeyManager::IsSecretKeyAlgorithm(KeyAlgorithm algorithm) {
    return algorithm == KeyAlgorithm::kAes128 || algorithm == KeyAlgorithm::kAes192 ||
           algorithm == KeyAlgorithm::kAes256 || algorithm == KeyAlgorithm::kDeS3;
}

bool KeyManager::IsKeyPairAlgorithm(KeyAlgorithm algorithm) {
    return algorithm == KeyAlgorithm::kRsa2048 || algorithm == KeyAlgorithm::kRsa3072 ||
           algorithm == KeyAlgorithm::kRsa4096 || algorithm == KeyAlgorithm::kEcdsaP256 ||
           algorithm == KeyAlgorithm::kEcdsaP384 || algorithm == KeyAlgorithm::kEcdsaP521;
}
