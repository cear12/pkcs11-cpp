...
KeyTransport::WrapResult KeyTransport::ExtractKeyTemplate(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
    ...
    functions->C_GetAttributeValue(session, key_handle, probe.data(), static_cast<CK_ULONG>(probe.size()));
    ...
    CK_RV rv = functions->C_GetAttributeValue(session, key_handle, probe.data(), static_cast<CK_ULONG>(probe.size()));
    if (rv != CKR_OK) {
...
KeyTransport::WrapResult KeyTransport::WrapKey(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
    ...
    switch (mechanism) {
        ...
        case WrapMechanism::kAesCbcPad:
            ...
            ck_mechanism = {CKM_AES_CBC_PAD, result.iv_.data(), static_cast<CK_ULONG>(result.iv_.size())};
            break;
        case WrapMechanism::kRsaPkcs:
            ck_mechanism = {CKM_RSA_PKCS, nullptr, 0};
            break;
        case WrapMechanism::kRsaOaep:
            oaep_params = {CKM_SHA256, CKG_MGF1_SHA256, CKZ_DATA_SPECIFIED, nullptr, 0};
            ck_mechanism = {CKM_RSA_PKCS_OAEP, &oaep_params, sizeof(oaep_params)};
            break;
    }

    CK_ULONG wrapped_len = 0;
    CK_RV rv = functions->C_WrapKey(session, &ck_mechanism, wrapping_key, key_to_wrap, nullptr, &wrapped_len);
    if (rv != CKR_OK) {
        throw std::runtime_error("C_WrapKey (sizing) failed: " + std::to_string(rv));
    }

    result.wrapped_key_.resize(wrapped_len);
    rv = functions->C_WrapKey(session, &ck_mechanism, wrapping_key, key_to_wrap, result.wrapped_key_.data(), &wrapped_len);
    if (rv != CKR_OK) {
...
CK_OBJECT_HANDLE KeyTransport::UnwrapKey(CK_SESSION_HANDLE session, CK_FUNCTION_LIST_PTR functions,
                                          const WrapResult& wrapped, CK_OBJECT_HANDLE unwrapping_key,
    ...
    if (!new_label.empty()) {
        ...
        if (!replaced) {
            unwrap_template.push_back({CKA_LABEL, label_storage.data(), static_cast<CK_ULONG>(label_storage.size())});
        }
    }

    CK_OBJECT_HANDLE unwrapped_key;
    CK_RV rv = functions->C_UnwrapKey(session, &ck_mechanism, unwrapping_key,
                                       const_cast<CK_BYTE*>(wrapped.wrapped_key_.data()),
                                       static_cast<CK_ULONG>(wrapped.wrapped_key_.size()), unwrap_template.data(),
                                       static_cast<CK_ULONG>(unwrap_template.size()), &unwrapped_key);
    if (rv != CKR_OK) {
