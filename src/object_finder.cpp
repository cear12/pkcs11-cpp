...
std::vector<CK_OBJECT_HANDLE> ObjectFinder::FindObjects(
  ...
  std::string cache_key = CacheKeyFor(criteria);
  {
    ...
    if (it != cache_.end() &&
        std::chrono::steady_clock::now() - it->second.cached_at_ <
            cache_timeout_) {
...
std::string ObjectFinder::CacheKeyFor(const SearchCriteria& criteria) {
  ...
  if (criteria.can_encrypt_)
    key << "canEncrypt=" << *criteria.can_encrypt_ << ";";
  return key.str();
}

void ObjectFinder::BuildSearchTemplate(
    const SearchCriteria& criteria, std::vector<CK_ATTRIBUTE>& search_template,
    std::vector<std::vector<CK_BYTE>>& attribute_data) {
  if (criteria.object_class_) {
    attribute_data.emplace_back(sizeof(CK_OBJECT_CLASS));
    *reinterpret_cast<CK_OBJECT_CLASS*>(attribute_data.back().data()) =
        *criteria.object_class_;
    search_template.push_back(
        {CKA_CLASS, attribute_data.back().data(),
         static_cast<CK_ULONG>(attribute_data.back().size())});
  }
  if (criteria.key_type_) {
    attribute_data.emplace_back(sizeof(CK_KEY_TYPE));
    *reinterpret_cast<CK_KEY_TYPE*>(attribute_data.back().data()) =
        *criteria.key_type_;
    search_template.push_back(
        {CKA_KEY_TYPE, attribute_data.back().data(),
         static_cast<CK_ULONG>(attribute_data.back().size())});
  }
  if (criteria.label_) {
    attribute_data.emplace_back(criteria.label_->begin(),
                                criteria.label_->end());
    search_template.push_back(
        {CKA_LABEL, attribute_data.back().data(),
         static_cast<CK_ULONG>(attribute_data.back().size())});
  }
  if (criteria.id_) {
    attribute_data.push_back(*criteria.id_);
    search_template.push_back(
        {CKA_ID, attribute_data.back().data(),
         static_cast<CK_ULONG>(attribute_data.back().size())});
  }
  AddBooleanAttribute(criteria.can_sign_, CKA_SIGN, search_template,
                      attribute_data);
  AddBooleanAttribute(criteria.can_encrypt_, CKA_ENCRYPT, search_template,
                      attribute_data);
}

void ObjectFinder::AddBooleanAttribute(
    const std::optional<bool>& value, CK_ATTRIBUTE_TYPE type,
    std::vector<CK_ATTRIBUTE>& search_template,
    std::vector<std::vector<CK_BYTE>>& attribute_data) {
  if (!value) return;
  attribute_data.emplace_back(sizeof(CK_BBOOL));
  *reinterpret_cast<CK_BBOOL*>(attribute_data.back().data()) =
      *value ? CK_TRUE : CK_FALSE;
  search_template.push_back(
      {type, attribute_data.back().data(),
       static_cast<CK_ULONG>(attribute_data.back().size())});
}
