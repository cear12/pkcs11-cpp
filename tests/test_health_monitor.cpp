...
TEST_CASE(
    "HealthMonitor::PerformComprehensiveTest passes against a healthy mock "
    "token",
    "[health_monitor]") {
  mock::Reset();
...
TEST_CASE("HealthMonitor::GetHealthReport flags low free memory as a warning",
          "[health_monitor]") {
  mock::Reset();
...
TEST_CASE(
    "HealthMonitor starts and stops cleanly without a health check ever having "
    "run",
    "[health_monitor]") {
  HealthMonitor monitor;
  auto report = monitor.GetHealthReport();
  REQUIRE(
      report.overall_healthy_);  // no slots checked yet => nothing unhealthy
