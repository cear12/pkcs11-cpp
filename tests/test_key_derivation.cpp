...
TEST_CASE("KeyDerivation::DeriveSp800_108Key is deterministic for the same inputs", "[key_derivation]") {
    ...
    CK_OBJECT_HANDLE derived1 = kd.DeriveSp800108Key(guard.Handle(), guard.Functions(), params);
    CK_OBJECT_HANDLE derived2 = kd.DeriveSp800108Key(guard.Handle(), guard.Functions(), params);
