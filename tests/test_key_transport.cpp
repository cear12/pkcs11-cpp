...
TEST_CASE("KeyTransport wrap/unwrap round-trips a key's usable attributes",
          "[key_transport]") {
  mock::Reset();
  ...
  CK_OBJECT_HANDLE payload_key =
      km.GenerateSecretKey(guard.Handle(), guard.Functions(), key_params);
  ...
  CK_OBJECT_HANDLE wrapping_key =
      km.GenerateSecretKey(guard.Handle(), guard.Functions(), wrapper_params);

  KeyTransport transport;
  auto wrapped =
      transport.WrapKey(guard.Handle(), guard.Functions(), payload_key,
                        wrapping_key, KeyTransport::WrapMechanism::kAesKeyWrap);
  REQUIRE_FALSE(wrapped.wrapped_key_.empty());

  CK_OBJECT_HANDLE restored = transport.UnwrapKey(
      guard.Handle(), guard.Functions(), wrapped, wrapping_key);
  REQUIRE(restored != CK_INVALID_HANDLE);
  REQUIRE(restored != payload_key);  // unwrap always produces a fresh object
}

TEST_CASE("KeyTransport::UnwrapKey applies a new label when one is given",
          "[key_transport]") {
  mock::Reset();
  SessionManager sm(mock::GetFunctionList(), 0);
  auto guard = sm.CreateSessionGuard();
  ...
  KeyTransport transport;
  auto wrapped = transport.WrapKey(guard.Handle(), guard.Functions(),
                                   payload_key, wrapping_key);
  CK_OBJECT_HANDLE restored =
      transport.UnwrapKey(guard.Handle(), guard.Functions(), wrapped,
                          wrapping_key, "renamed-label");

  AttributeManager attrs;
  auto read_back =
      attrs.ReadObjectAttributes(guard.Handle(), guard.Functions(), restored);
