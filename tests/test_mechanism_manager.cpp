...
TEST_CASE("MechanismManager discovers the mock module's mechanism table",
          "[mechanism_manager]") {
  mock::Reset();
  MechanismManager mm;
...
TEST_CASE("MechanismManager::SelectBestMechanism respects operation + key type",
          "[mechanism_manager]") {
  mock::Reset();
  MechanismManager mm;
  mm.DiscoverMechanisms(mock::GetFunctionList(), 0);
...
TEST_CASE(
    "MechanismManager::SelectBestMechanism returns nullopt for an unknown slot",
    "[mechanism_manager]") {
  MechanismManager mm;  // DiscoverMechanisms() never called
  REQUIRE_FALSE(mm.SelectBestMechanism(0, "encrypt", CKK_AES).has_value());
}

TEST_CASE("CreateOptimizedMechanism fills in OAEP defaults",
          "[mechanism_manager]") {
  MechanismManager mm;
