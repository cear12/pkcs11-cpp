...
namespace {
CK_OBJECT_HANDLE MakeAesKey(CK_SESSION_HANDLE session,
...
TEST_CASE("ObjectFinder finds objects by label", "[object_finder]") {
  ...
  ObjectFinder finder;
  auto matches =
      finder.FindObjects(guard.Handle(), guard.Functions(),
                         ObjectFinder::SearchCriteria().WithLabel("alpha"));
  REQUIRE(matches.size() == 1);
}

TEST_CASE("ObjectFinder returns nothing for a label that doesn't exist",
          "[object_finder]") {
  mock::Reset();
  SessionManager sm(mock::GetFunctionList(), 0);
  auto guard = sm.CreateSessionGuard();
  ...
  auto matches = finder.FindObjects(
      guard.Handle(), guard.Functions(),
      ObjectFinder::SearchCriteria().WithLabel("does-not-exist"));
  REQUIRE(matches.empty());
}

TEST_CASE("ObjectFinder caches results and ClearCache forces a re-search",
          "[object_finder]") {
  mock::Reset();
  SessionManager sm(mock::GetFunctionList(), 0);
  auto guard = sm.CreateSessionGuard();
  ...
  MakeAesKey(guard.Handle(), guard.Functions(),
             "cached-key");  // a second match, added after caching
  auto second = finder.FindObjects(guard.Handle(), guard.Functions(), criteria);
  REQUIRE(second.size() ==
          first.size());  // still cached, doesn't see the new object yet
